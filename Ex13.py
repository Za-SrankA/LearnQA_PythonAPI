...
class TestExample13:
    ...
    def test_check_user_agent(self,user_agent,platform,browser,device):
        ...
        assert platform == actual_platform,f"Parameter 'platform' in response is not correct! Now: {actual_platform}. Expected: {platform}"
        assert browser == actual_browser,f"Parameter 'browser' in response is not correct! Now: {actual_browser}. Expected: {browser}"
        assert device == actual_device,f"Parameter 'device' in response is not correct! Now: {actual_device}. Expected: {device}"