...
class TestUserEdit(BaseCase):
    ...
    def test_edit_just_created_user_by_other_authorized_user(self):
        ...
        Assertions.assert_json_value_by_name(
            ...
            "firstName",
            user1_first_name,
            "Wrong name of the user after incorrect edit. 'firstName' can not be edited"
        )

    incorrect_params = {"email","firstName"}
    @allure.description("Проверка невозможности авторизованным юзером менять свои данные на неккоректные значения")
    ...
    def test_edit_just_created_user_with_incorrect_data(self,incorrect_parameter):
        ...
        AllureDescription.add_step(f"Регистрация пользоватем,данные которого будут редактироваться.")
        # REGISTER
        register_data = self.prepare_registration_data()
        response1 = MyRequests.post(
            "/user/",
        ...
        AllureDescription.add_step("Авторизация созданным пользователем")
        # LOGIN
        login_data = {
            "email" : email,
            "password" : password
        ...
        AllureDescription.add_step("Редактирование данных авторизованного пользователя")
        # EDIT
